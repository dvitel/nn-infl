...
for run_id in {0..10}; do
    ...
    srun --export=ALL,INFL_SEED=$run_id,INFL_CWD=$task_cwd python ~/infl/src/exp.py finetune2 --task=$task --filter-method=infl --infl-method=datainf --module-pattern=.\*\\.layer\\.\(\[0-9\]\|1\[0-5\]\)\\..\*\\.lora_\(A\|B\)\\..\*
