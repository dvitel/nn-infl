...
def lissa_fn(module_train_grad, module_avg_val_grads, lambda_const_param=10, n_iteration=10, alpha_const=1.):
    ...
    for _ in range(n_iteration):
        new_running_hvp = module_avg_val_grads + running_hvp - alpha_const * torch.sum((torch.sum(module_train_grad * running_hvp, dim=-1).view(-1, 1) * module_train_grad - lambda_const * running_hvp) / n_train, dim=0)
        del running_hvp
